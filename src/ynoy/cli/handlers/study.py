...
from ynoy.cli.handlers.study_local_authenticator import (
    enroll_local_authenticator,
    local_authenticator_status,
)
...
def handle_study(args: argparse.Namespace, context: CommandContext) -> dict[str, object]:
    handlers = {
        ...
        "enroll-local-authenticator": enroll_local_authenticator,
        "local-authenticator-status": local_authenticator_status,
    }
...
def _prepare(args: argparse.Namespace, context: CommandContext) -> dict[str, object]:
    ...
    message_tr, next_step_tr = status_guidance_tr(manifest.status)
    return {
...
def _status(args: argparse.Namespace, context: CommandContext) -> dict[str, object]:
    ...
    if purge.failed_count:
        message_tr, next_step_tr = status_guidance_tr("expiry_purge_incomplete")
        return {
    ...
    message_tr, next_step_tr = status_guidance_tr(status)
    return {
        "status": status,
        "message_tr": message_tr,
        "next_step_tr": next_step_tr,
...
def _delete(args: argparse.Namespace, context: CommandContext) -> dict[str, object]:
    ...
    message_tr, next_step_tr = status_guidance_tr("derived_study_deleted")
    return {
...
def _seal_labels(args: argparse.Namespace, context: CommandContext) -> dict[str, object]:
    ...
    message_tr, next_step_tr = status_guidance_tr("annotation_labels_sealed_not_persona_quality")
    return {
