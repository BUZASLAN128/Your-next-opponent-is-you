...
def _study_parser(commands: argparse._SubParsersAction[argparse.ArgumentParser]) -> None:
    ...
    harvest.add_argument("codex_root")
    harvest.add_argument("source_study_id")
    harvest.add_argument("--synthetic", action="store_true")
    resume_harvest = subcommands.add_parser("resume-harvest")
    ...
    seal_authorship.add_argument("--checkpoint-sha256", required=True)
    seal_authorship.add_argument("--confirm-all-self", action="store_true", required=True)
    seal_authorship.add_argument("--synthetic", action="store_true")
